...
/* ===== RV FLOWERCRAFT CLOSURE SYSTEM ===== */
(function(){

  const PASSWORD = "Raffi";
  const KEY = "rv_flowercraft_closure_messages";

  const overlay = document.getElementById("temporaryClosure");
  const message = document.getElementById("closureMessage");
  const send = document.getElementById("closureSend");
  const view = document.getElementById("closureView");

  const login = document.getElementById("closureLogin");
  const password = document.getElementById("closurePassword");
  const unlock = document.getElementById("closureUnlock");
  const error = document.getElementById("closureError");

  const inbox = document.getElementById("closureInbox");
  const messages = document.getElementById("closureMessages");
  ...
  function renderMessages(){
    ...
    if(!data.length){
      messages.innerHTML =
        '<div class="closure-message">Belum ada pesan.</div>';
      return;
    }

    messages.innerHTML = data.map(function(item){
  ...
  document.getElementById("closureLoginClose")
    .addEventListener("click",function(){
      login.classList.remove("show");
    });

  document.getElementById("closureInboxClose")
    .addEventListener("click",function(){
      inbox.classList.remove("show");
    });

  document.getElementById("closureClear")
    .addEventListener("click",function(){
...
/* =========================================================
   RV FLOWERCRAFT - CLOSURE MEGA EFFECTS
========================================================= */

(function(){

  const PASSWORD = "raffi1212";
  ...
  let sky =
    closure.querySelector(".closure-sky");

  if(!sky){

    sky = document.createElement("div");

    sky.className = "closure-sky";
  ...
  const unlock =
    document.getElementById("closureUnlock");

  const password =
    document.getElementById("closurePassword");

  const error =
    document.getElementById("closureError");
  ...
  if(unlock){
    ...
    const newUnlock =
      unlock.cloneNode(true);

    unlock.parentNode.replaceChild(
      newUnlock,
      unlock
    );


    newUnlock.addEventListener(
      "click",
      function(){
        ...
        if(password.value === PASSWORD){
          ...
          document
            .getElementById("closureLogin")
            .classList.remove("show");

          document
            .getElementById("closureInbox")
            .classList.add("show");

          if(typeof window.renderClosureMessages === "function"){
            window.renderClosureMessages();
          }
  ...
  const sender =
    document.getElementById("closureSender");
    ...
    newSend.addEventListener(
      ...
      function(){
        ...
        if(!text){
          ...
          alert(
            "💌 Silahkan tulis pesan terlebih dahulu."
          );
        ...
        const key =
          "rv_flowercraft_closure_messages";
...
/* RV_FLOWERCRAFT_CLOSURE_JS_START */

(function () {

  const CLOSURE_PASSWORD = "raffi1212";
  const STORAGE_KEY = "rv_flowercraft_closure_messages";

  const $ = (id) => document.getElementById(id);

  const closure = $("temporaryClosure");
  const sender = $("closureSender");
  const message = $("closureMessage");
  const sendBtn = $("closureSend");
  const viewBtn = $("closureView");

  const login = $("closureLogin");
  const inbox = $("closureInbox");

  const password = $("closurePassword");
  const unlock = $("closureUnlock");
  const error = $("closureError");

  const messagesBox = $("closureMessages");
  const loginClose = $("closureLoginClose");
  const inboxClose = $("closureInboxClose");
  const clearBtn = $("closureClear");
  ...
  function renderMessages() {
    ...
    if (!list.length) {
      messagesBox.innerHTML =
        '<div class="rv-empty-message">Belum ada pesan masuk.</div>';
      return;
    }

    messagesBox.innerHTML = list
      .slice()
      .reverse()
      .map((item) => {
        ...
        return `
          <div class="rv-message-item">
            <div class="rv-message-name">
              ${escapeHTML(item.name || "Tanpa nama")}
            </div>

            <div class="rv-message-date">
              ${escapeHTML(dateText)}
            </div>
  ...
  function openLogin() {
    ...
    password.value = "";
    error.textContent = "";
    login.classList.add("rv-modal-show");
  ...
  function closeLogin() {
    ...
    if (login) {
      login.classList.remove("rv-modal-show");
    }
  ...
  function openInbox() {
    ...
    inbox.classList.add("rv-modal-show");
  ...
  function closeInbox() {
    ...
    if (inbox) {
      inbox.classList.remove("rv-modal-show");
    }
  ...
  if (sendBtn) {
    ...
    sendBtn.addEventListener("click", function () {
      ...
      if (!name) {
        sender.focus();
        sender.style.borderColor = "#b77d72";
      ...
      if (!text) {
        message.focus();
        message.style.borderColor = "#b77d72";
  ...
  if (viewBtn) {
    viewBtn.addEventListener("click", openLogin);
  }
  ...
  if (unlock) {
    ...
    unlock.addEventListener("click", function () {
      ...
      if (password.value === CLOSURE_PASSWORD) {
  ...
  [login, inbox].forEach((modal) => {
    ...
    modal.addEventListener("click", function (event) {
      ...
      if (event.target === modal) {
        modal.classList.remove("rv-modal-show");
      }
  ...
  function showClosure() {
    ...
    closure.classList.add("rv-show");

    document.body.classList.add("rv-closure-active");

  }

  /*
   * Tampilkan setelah halaman selesai loading.
   * Delay kecil supaya halaman utama/loader tetap sempat muncul.
   */
  function startClosure() {

    setTimeout(showClosure, 1600);
  ...
  if (document.readyState === "complete") {
    ...
    window.addEventListener("load", startClosure, {
      once: true
    });
  }
