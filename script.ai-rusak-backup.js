...
(function () {
  ...
  function addUserMessage(text) {
    ...
    row.className = "skybot-message skybot-user";
    ...
    row.querySelector(".skybot-bubble").textContent = text;
  ...
  function addBotMessage(text) {
    ...
    const row = document.createElement("div");
    row.className = "skybot-message skybot-bot";
  ...
  function addImageMessage(prompt, image) {
    ...
    avatar.className = "skybot-avatar";
    avatar.textContent = "🎨";

    const bubble = document.createElement("div");
    bubble.className = "skybot-bubble skybot-image-bubble";

    const title = document.createElement("div");
    title.textContent = "Gambar AI";
    ...
    if (image && image instanceof HTMLImageElement) {
      ...
      addBotMessage(
        "Gambar berhasil dibuat, tetapi hasilnya tidak dapat ditampilkan."
      );
  ...
  function addThinking(text = "SkyBot sedang berpikir...") {
    ...
    row.className =
      "skybot-message skybot-bot skybot-thinking";
  ...
  function setImageMode(active) {
    ...
    const input = $("skybotInput");

    const imageButton = $("skybotImage");

    if (input) {

      input.placeholder = active
        ? "Tulis prompt gambar AI..."
        : "Tanyakan sesuatu ke SkyBot...";
  ...
  async function sendChat(text) {
    ...
    try {
      ...
      if (!answer) {
        ...
        throw new Error(
          "SkyBot tidak memberikan jawaban."
        );
  ...
  async function generateImage(prompt) {
    ...
    if (
      ...
      addBotMessage(
        "Puter.js belum siap. Coba refresh halaman lalu ulangi."
      );
    ...
    try {
      ...
      addBotMessage(
        "Maaf, gambar belum berhasil dibuat. Coba prompt lain."
      );
  ...
  function renderWelcome() {
    ...
    if (box.dataset.skybotWelcome === "1") return;

    box.dataset.skybotWelcome = "1";

    box.innerHTML = `
      <div class="skybot-welcome">

        <div class="skybot-welcome-icon">🤖</div>

        <strong>Halo, saya SkyBot 👋</strong>

        <p>
          Asisten virtual RV FLOWERCRAFT.
          Kamu bisa bertanya atau membuat gambar AI.
        </p>

        <div class="skybot-suggestions">

          <button
            type="button"
            data-sky-question="Apa saja yang bisa kamu bantu?">
            💬 Apa yang bisa kamu bantu?
          </button>

          <button
            type="button"
            data-sky-question="Bagaimana cara memesan buket?">
            💐 Cara pesan
          </button>

          <button
            type="button"
            data-sky-image-prompt="Buket bunga pink cantik untuk RV FLOWERCRAFT, foto realistis">
            🎨 Coba buat gambar
          </button>
  ...
  function init() {
    ...
    const input = $("skybotInput");
    const send = $("skybotSend");
    const plus = $("skybotPlus");
    const menu = $("skybotPlusMenu");
    const mic = $("skybotMic");
    const file = $("skybotFile");
    const photo = $("skybotPhoto");
    const fileBtn = $("skybotFileBtn");
    const imageBtn = $("skybotImage");
    const attachment = $("skybotAttachment");

    if (!input || !send) {

      console.warn(
        "SkyBot: composer belum ditemukan."
      );
    ...
    if (mic && SpeechRecognition) {
      ...
      mic.addEventListener(
        ...
        function () {
          ...
          addBotMessage(
            "Fitur mikrofon belum didukung browser ini."
          );
