...
function productPriceHTML(p){
  ...
  if(p.promo_active && Number(p.promo_price)>0 && Number(p.promo_price)<Number(p.price)){
    return `<div class="promo-price">
      <span class="price-old">${normal}</span>
...
/* Harga pada kartu katalog */
const originalProductCard=window.productCard;

if(typeof originalProductCard==="function"){
  window.productCard=function(p){
    let html=originalProductCard(p);

    const oldPrice=rupiah(p.price);
