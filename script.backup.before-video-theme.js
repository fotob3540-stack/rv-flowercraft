...
/* =========================================
   MUSIC THEME — DARK GLOW LUXURY
   Lagu 1 & 2 = normal
   Lagu 3 & 4 = dark glow
========================================= */

function updateMusicTheme(){
  const video = document.getElementById("musicBgVideo");
  const luxury = trackIndex >= 2;

  document.body.classList.toggle("music-video-mode", luxury);

  if(!video) return;

  const videoSrc = "assets/music-bg.mp4";
  ...
  if(luxury){
    ...
    // Fade out dulu, lalu kosongkan video
    document.body.classList.remove("music-video-mode");

    setTimeout(()=>{
      if(!document.body.classList.contains("music-video-mode")){
        video.pause();
...
function updateMusicUI(){const playing=!musicAudio.paused;musicPlayer.classList.toggle("playing",playing);playBtn.textContent=playing?"Ⅱ":"▶";muteBtn.textContent=musicAudio.muted?"🔇":"🔊";musicPlayer.title=tracks[trackIndex].name;updateMusicTheme()}
